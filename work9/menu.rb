class Menu
  ...
  def wagons_at_train
    Train.all.each { |tr| puts "Train: #{tr}" }

      tr.each_wagons { |wag| puts "#{wag}" }
  end

  def trains_at_station
    Station.all.each { |st| puts "Station: #{st}" }

    st.each_trains { |tr| puts "#{tr}" }
  end

  def take_wag_seat
    puts 'Choose a wagon'
    PassengerWagon.all.each_with_index { |wag, i| puts "#{i + 1} -- #{wag}" }
  ...
  def take_wag_vol
    puts 'Choose a wagon'
    CargoWagon.all.each_with_index { |wag, i| puts "#{i + 1} -- #{wag}" }
  ...
  def add_train_in_st
    ...
    Station.all.each_with_index { |st, i| puts "#{i + 1} -- #{st}" }
    choosed_station = gets.chomp.to_i

    puts 'Choose a train'
    Train.all.each_with_index { |tr, i| puts "#{i + 1} -- #{tr}" }
  ...
  def create_route
    ...
    if Station.all.size >= 2
      Station.all.each_with_index { |st, i| puts "#{i + 1} -- #{st.st_name}" }
  ...
  def create_train
    return 'You need add route' if @created_route.size.zero?
  ...
  def remove_station_menu
    return 'You need add station' if @created_route.size.zero?
